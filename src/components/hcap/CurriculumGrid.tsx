...
export function CurriculumGrid() {
    return (
        <section className="py-[100px] px-[20px] md:px-[40px] bg-[#f9fffe]">
            <div className="max-w-[1200px] mx-auto">
                <div className="text-center mb-[60px]">
                    <h2 className="text-3xl md:text-4xl font-bold font-heading text-triverge-blue">
                        What you learn
                    </h2>
                    <p className="mt-4 text-charcoal/60 max-w-[600px] mx-auto">
                ...
                >
                    {CURRICULUM_ITEMS.map((item, idx) => (
                        <motion.div
                            key={idx}
                            variants={cardFlip}
                            className="bg-white rounded-[16px] p-[20px] flex items-center gap-[15px] shadow-sm hover:shadow-lg transition-all duration-300 group hover:-translate-y-1 border border-gray-100"
                        >
                            ...
                            <span className="font-heading font-medium text-[#212121] text-lg">
                                {item.title}
                            </span>
                        </motion.div>
                    ))}
