...
export function FAQSection() {
    ...
    return (
        <section className="py-[80px] px-[20px] md:px-[40px] bg-[#f9fffe]">
            <div className="max-w-[800px] mx-auto">
                <div className="text-center mb-[60px]">
                    <h2 className="text-3xl font-bold font-heading text-triverge-blue">Common Questions</h2>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    {FAQS.map((faq, idx) => (
                        <div
                            key={idx}
                            className="bg-white border border-gray-100 rounded-[16px] overflow-hidden transition-all duration-300 hover:shadow-md cursor-pointer"
                            onClick={() => setOpenIndex(openIndex === idx ? null : idx)}
                        >
                            <div className="p-6 flex items-center justify-between gap-4">
                                <h3 className={cn(
                                    "text-lg font-bold font-heading transition-colors",
                                    openIndex === idx ? "text-healing-teal" : "text-triverge-blue"
                                )}>
                                    {faq.q}
                                </h3>
                                <div className={cn(
                                    "w-[32px] h-[32px] rounded-full flex items-center justify-center transition-all duration-300",
                                    openIndex === idx ? "bg-healing-teal text-white rotate-45" : "bg-gray-100 text-charcoal"
                                )}>
                            ...
                            <AnimatePresence>
                                {openIndex === idx && (
                                    ...
                                    >
                                        <div className="px-6 pb-6 pt-0 text-charcoal/80 font-body leading-relaxed border-t border-transparent">
                                            {faq.a}
                                        </div>
                                    </motion.div>
                                )}
